...
keyword_df <- read.csv("Keyword_data.csv")
#Cleaning the data frame by removing unwanted and empty rows
keyword_df<- keyword_df[-c(1,2,11,12,13,23,24,25,34,35,36,45,46,47,56,57,58),]
#Creating data frame
keyword_df <- data.frame(lapply(keyword_df, str_to_title))

#Finding unique keywords using stack
stacked_keywords <- stack(keyword_df[,2:13])

#Finding unique keywords from the stacked keywords
unique_keywords<-sort(unique(stacked_keywords$values)) 
unique_keywords<-unique_keywords[c(-1)]

#Creating an adjacency matrix
result_matrix <- matrix(0, nrow=length(unique_keywords), ncol=length(unique_keywords))
colnames(result_matrix) <- unique_keywords
rownames(result_matrix) <- unique_keywords
for(i in 1:length(keyword_df$Keyword.2)){
  temp <- unlist(keyword_df[i,])
  temp <- temp[temp != ""]
  keyword_data_list <- combn(temp,2)
  for(j in 1:length(keyword_data_list[1,])){
    row_index <- which(rownames(result_matrix) == (keyword_data_list[1,j]))
    col_index <- which(colnames(result_matrix) == (keyword_data_list[2,j]))
    result_matrix[row_index,col_index] <- result_matrix[row_index,col_index]+1
    result_matrix[col_index,row_index] <- result_matrix[col_index,row_index]+1
  }
}

#Converting into a weighted network
network <- graph_from_adjacency_matrix(result_matrix, weighted =  TRUE)
plot(network, n=248, vertex.size=4, vertex.label = NA)
...
node_pairs <- get.data.frame(network)
node_pairs <- node_pairs[order(-node_pairs$weight),]
head(node_pairs, 10)

#Plotting Average Strength on Y-axis and Degree on X-axis
avg_val <- tapply(node_strength, node_degree, mean)
avg_val <- data.frame(avg_val)
avg_stg_vs_deg <- cbind(degree=rownames(avg_val), avg_val)
row.names(avg_stg_vs_deg) <- NULL
plot(avg_stg_vs_deg, type='p', pch = 19, main='Average Strength vs Degree', xlab='Degree', ylab='Average Strength', frame = FALSE)
lines(lowess(avg_stg_vs_deg$degree, avg_stg_vs_deg$avg_val), col = "red")
